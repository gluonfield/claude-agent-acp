...
type InterruptReceipt = { still_queued: string[] };
type Query = Omit<SDKQuery, "interrupt"> & {
  interrupt(): Promise<void | InterruptReceipt>;
};
          ...
          case "system":
            switch (message.subtype) {
              case "init": {
                ...
                const capabilities = (message as typeof message & { capabilities?: string[] })
                  .capabilities;
                if (capabilities?.includes("msg_lifecycle_v1")) {
...
export function toAcpNotifications(
  ...
  for (const chunk of content) {
    ...
    if (update) {
      ...
      const providerSubagent = jazProviderSubagent(update, options?.parentToolUseId);
      if (providerSubagent) {
        update._meta = {
          ...update._meta,
          jaz: { providerSubagent },
        };
      }
      applyMessageId(update, options?.messageId);
